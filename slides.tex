...
\begin{frame}
  \hero{Barriers to \color{b-purple} git}
  \lowerhalf{%
    \note{Git is famously difficult to learn.  There are arguably
      easier tools out there (e.g., mercurial) but few with as wide
      adoption by scientists.  99\% of the time, git is very easy to
      use; add and commit, add and commit, push.}%
    \includegraphics<1>[width=.7\paperwidth]{shots/git-hard}%
    \only<2>{%
...
\begin{frame}
  ...
  \bottomleft{%
    \only<1>{\href{http://xkcd.com/1296/}{http://xkcd.com/1296/}}}
...
\begin{frame}
  \minionhigh{\LARGE CI = Continuous Integration}
  \lowerhalf{
    \begin{minipage}{\textwidth}
      \begin{enumerate}
      \item Commit changes
      \item \only<1>{Make sure nothing breaks}\only<2>{Push to GitHub\phantom{g}}
      \end{enumerate}
    \end{minipage}
  }
  \note{}
...
\begin{frame}
  \minionhigh{.travis.yml}
  \bottomhanging{\includegraphics[width=.8\paperwidth]{snippets/travis_config}}
  \bottomleft{travis tool: \href{https://github.com/craigcitro/r-travis/}{https://github.com/craigcitro/r-travis/}}
\end{frame}

\begin{frame}
  \minionhigh{%
    \only<1>{Spin up virtual machine}%
    \only<2>{Install dependencies}%
    \only<3>{Download and pre-process data}%
    \only<4>{Run \color{b-orange}knitr\phantom{p}}%
    \only<5>{Compile manuscript}}
  \bottomhanging{%
...
\begin{frame}
  ...
  \bottomleft{\href{http://rstudio.github.io/packrat}{http://rstudio.github.io/packrat}}
...
\begin{frame}
  \inthemiddle{\includegraphics[width=\paperwidth]{pics/alice}}
\end{frame}
...
\begin{frame}
  \begin{tikzpicture}[remember picture, overlay]
    ...
    ($(current page.east) + (-0.15\paperwidth, -.17\paperheight)$) {%
      46\%
    };
  \end{tikzpicture}
\end{frame}
