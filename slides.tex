...
\begin{frame}
  ...
  \lowerhalf{
    \only<1>{Don't do those things}%
    \only<2>{Reproducibility depends on\\
      tools \& workflows \textbf{around} R}
  }
  \note{
...
\begin{frame}
  ...
  \bottomleft{\href{http://yihui.name/knitr}{yihui.name/knitr}}
...
\begin{frame}
  ...
  \bottomleft{\href{http://git-scm.com}{git-scm.com}}
...
\begin{frame}
  ...
  \only<1>{\lowerhalf{\LARGE R + {\color{b-purple} git} = nice}\phantom{\heart}}%
  \only<2>{\lowerhalf{\LARGE R + {\color{b-purple} git} + {\color{b-orange}BitBucket} = {\color{b-pink}\heart}}}
  \only<3>{\lowerhalf{\LARGE R + {\color{b-purple} git} + {\color{b-orange}GitHub} = {\color{b-pink}\heart}}}%
\end{frame}
...
\begin{frame}
  ...
  \only<1>{\bottomleft{Jack William Bell: http://perl.plover.com/classes/git/samples/slide034.html}}
\end{frame}
...
% TODO: Get something here, or drop this slide
\begin{frame}
  \hero{Version control\\[-.7ex]\color{b-purple} git}
  \lowerhalf{Prospects?}
...
\begin{frame}
  ...
  \lowerhalf{
    ...
    \only<3>{Currently looking for a\\[-.4ex]modern, accessible replacement}%
  }
  \note{
    The bad:
    \begin{itemize}
...
\begin{frame}
  ...
  \bottomleft{\href{http://travis-ci.org}{travis-ci.org}}
  \note{
  ...
  \lowerhalf{
    \begin{minipage}{\textwidth}
      ...
      \item \only<1>{Make sure nothing breaks}%
        \only<2>{Push to GitHub\phantom{g}}
      \end{enumerate}
    \end{minipage}
  }
  \note{}
...
\begin{frame}
  \minionhigh{%
    \only<1>{Spins up virtual machine}%
    \only<2>{Installs dependencies}%
    \only<3>{Downloads and pre-process data}%
    \only<4>{Runs \color{b-orange}knitr\phantom{p}}%
    \only<5>{Compiles manuscript}}
  \bottomhanging{%
...
\begin{frame}
  ...
  \bottomleft{travis-CI for R: \href{https://github.com/craigcitro/r-travis/}{github.com/craigcitro/r-travis/}}
\end{frame}
...
\begin{frame}
  ...
  \lowerhalf{%
    \only<1>{Project must be reproducible \textbf{first}}%
...
\begin{frame}
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \node<2>[anchor=south east] at (current page.south east) {
      \small\color{white}%
      \href{http://richfitz.github.io/wood}{richfitz.github.io/wood}
    };
  \end{tikzpicture}
\end{frame}
...
\begin{frame}
  ...
  \lowerhalf{
    \only<1>{It's not just good -- it's good enough}%
    \only<2>{Good faith effort at documenting requirements\\
      makes it \textbf{much} easier to pick up}%
  }
\end{frame}

\begin{frame}
  \herohigh{\color{b-pink}Acknowledgements}
...
\begin{frame}
  ...
  \begin{tikzpicture}[remember picture, overlay]%
    ...
    ($(current page.west) + (0.05\paperwidth, .17\paperheight)$) {
      ...
      travis-CI&\hrefp{http}{travis-ci.org}\\
      travis-CI for R&\hrefp{https}{github.com/craigcitro/r-travis/}\\
    \end{tabular}
  };
  \end{tikzpicture}
\end{frame}
